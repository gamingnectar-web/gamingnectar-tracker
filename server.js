...
// =====================================================================
// SHOPIFY HELPERS (Kept exactly as you have them)
// =====================================================================
...
// =====================================================================
// 🛠️ AFTERSHIP HELPERS (BLIND AUTO-DETECT)
// =====================================================================
async function callAfterShip(endpoint, method = 'GET', body = null) {
  ...
  // 🚀 THE FIX: We only send the tracking number. We let AfterShip's AI detect Royal Mail.
...
app.get('/api/track', async (req, res) => {
  ...
  try {
    ...
    // Step 1: Check if AfterShip already has this package
    let data = await callAfterShip(`trackings?tracking_numbers=${cleanNumber}`);
    let track = data.data?.trackings?.[0];

    // Step 2: If it doesn't exist, register it blindly
    if (!track) {
      console.log(`📦 Registering new package: ${cleanNumber}`);
      await callAfterShip('trackings', 'POST', { tracking_number: cleanNumber });
      
      // Return PENDING so the frontend doesn't crash while AfterShip fetches the timeline
      return res.json({ status: 'PENDING', history: [] }); 
    }

    // Step 3: Return the timeline
    return res.json({
...
app.post('/api/webhooks/fulfillment', async (req, res) => {
  ...
  try {
    ...
    // Webhook registers it blindly too
    const result = await callAfterShip('trackings', 'POST', { tracking_number: num });
